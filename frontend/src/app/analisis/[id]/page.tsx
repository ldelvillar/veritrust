...
export default function AnalisisDetailPage() {
  ...
  useEffect(() => {
    if (loading) document.title = 'Cargando análisis | VeriTrust';
    else document.title = 'Resultado del análisis | VeriTrust';
  ...
  useEffect(() => {
    if (!analysisId) {
      setError('No se encontró un id de análisis válido.');
      setLoading(false);
      return;
    }
