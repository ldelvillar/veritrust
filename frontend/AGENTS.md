...
- **Verdict vocabulary** — the three verdict states are always "Verdadero"/"Falso"/"Dudoso", sourced from `VERDICT_LABEL` in `frontend/src/components/analysis-result/format.ts`; never hardcode alternate wording (e.g. "Fiable", "Engañoso") in a new screen. A row's or card's tone and verdict wording follow the `verdict` field the backend sends, never a cut on `credibility` (a true verdict's credibility can sit in the 40s); credibility is shown only as a number.
