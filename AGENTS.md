...
User (browser)
  ...
  → enqueue run_analysis on Redis ─────────────→ run_analysis(analysis_id, notify_email)
  → Return {status: "pending", analysis_id}        → load content from the row; URL/file text extraction if needed
                                                   → LangGraph pipeline:
