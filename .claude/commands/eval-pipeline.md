...
`backend/ml/evaluate_pipeline.py`; you run it and interpret the output.
...
- This is SLOW: each sample runs several sequential Ollama calls plus the
  evidence lookups. Default `--limit` is 30; keep it small unless the user asks
  for a full run.

STEPS:

1. From `backend/`, run the harness, forwarding any `$ARGUMENTS`:
   `uv run --extra ml python -m ml.evaluate_pipeline $ARGUMENTS`
...
3. Summarize for the user:
   ...
   - Whether the failure looks like it originates in the Extractor (no claims),
     the Translator, the Investigator's evidence retrieval, or the Health Expert
     verdict — reason from the evidence, don't guess.
