...
- **`app/core/`** — Cross-cutting logic the routes call: verdict and credibility (`credibility.py`), history CSV serialisation including the formula-injection guard (`history_export.py`), enqueueing `run_analysis` with the rollback to `failed` shared by the routes and MCP (`analysis_jobs.py`; never call `enqueue_job("run_analysis", …)` directly). Routes fetch and respond; formatting lives here.
